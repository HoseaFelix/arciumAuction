...
// Computation definition offsets for each MPC function
const COMP_DEF_OFFSET_INIT_AUCTION_STATE: u32 = comp_def_offset("init_auction_state");
const COMP_DEF_OFFSET_PLACE_BID_V2: u32 = comp_def_offset("place_bid_v2");
...
const AUCTION_ENCRYPTED_STATE_OFFSET: u32 = 8 + 1 + 32 + 1 + 1 + 8 + 8 + 1 + 16;
const AUCTION_ENCRYPTED_STATE_SIZE: u32 = 32 * 5;

declare_id!("MN1xF8e7iUD3wQYsaJSmA2u5BuUGK5xyN8aBP5vLwZ7");
...
pub mod auction {
    ...
    // ============================================================================
    // Computation Definition Initialization Instructions
    // ============================================================================
    ...
    pub fn init_auction_state_comp_def(ctx: Context<InitAuctionStateCompDef>) -> Result<()> {
        init_comp_def(
            ...
            Some(CircuitSource::OffChain(OffChainCircuitSource {
                source: "https://epwjbgtsijjbjjslhrxz.supabase.co/storage/v1/object/public/arcium-circuits/init_auction_state.arcis"
                    .to_string(),
    ...
    pub fn init_place_bid_comp_def(ctx: Context<InitPlaceBidCompDef>) -> Result<()> {
        init_comp_def(
            ctx.accounts,
            Some(CircuitSource::OffChain(OffChainCircuitSource {
                source: "https://epwjbgtsijjbjjslhrxz.supabase.co/storage/v1/object/public/arcium-circuits/place_bid_v2.arcis"
                    .to_string(),
                hash: circuit_hash!("place_bid_v2"),
            })),
            None,
        )?;
        Ok(())
    ...
    pub fn init_determine_winner_first_price_comp_def(
        ...
        init_comp_def(
            ctx.accounts,
            Some(CircuitSource::OffChain(OffChainCircuitSource {
                source: "https://epwjbgtsijjbjjslhrxz.supabase.co/storage/v1/object/public/arcium-circuits/determine_winner_first_price.arcis"
                    .to_string(),
    ...
    pub fn init_determine_winner_vickrey_comp_def(
        ...
        init_comp_def(
            ctx.accounts,
            Some(CircuitSource::OffChain(OffChainCircuitSource {
                source: "https://epwjbgtsijjbjjslhrxz.supabase.co/storage/v1/object/public/arcium-circuits/determine_winner_vickrey.arcis"
                    .to_string(),
    ...
    /// Callback for auction state initialization
    ...
    pub fn init_auction_state_callback(
        ...
        let item_name = ctx.accounts.auction.item_name.clone();
    ...
    pub fn place_bid(
        ...
        require!(auction.state_nonce != 0, ErrorCode::AuctionStateNotReady);
        ...
        let args = ArgBuilder::new()
            ...
            .account(
                ...
                AUCTION_ENCRYPTED_STATE_OFFSET,
                AUCTION_ENCRYPTED_STATE_SIZE,
            )
            .build();
        ...
        queue_computation(
            ...
            computation_offset,
            args,
            vec![PlaceBidV2Callback::callback_ix(
                computation_offset,
                &ctx.accounts.mxe_account,
                &[CallbackAccount {
                    pubkey: ctx.accounts.auction.key(),
    ...
    #[arcium_callback(encrypted_ix = "place_bid_v2")]
    pub fn place_bid_v2_callback(
        ctx: Context<PlaceBidV2Callback>,
        output: SignedComputationOutputs<PlaceBidV2Output>,
    ) -> Result<()> {
        let o = match output.verify_output(
            &ctx.accounts.cluster_account,
            &ctx.accounts.computation_account,
        ) {
            Ok(PlaceBidV2Output { field_0 }) => field_0,
            Err(_) => return Err(ErrorCode::AbortedComputation.into()),
        };
    ...
    /// Determine winner (first-price auction)
    ...
    /// Determine winner (Vickrey auction)
...
pub struct CreateAuction<'info> {
    ...
    #[account(
        ...
        seeds = [b"auction", authority.key().as_ref(), &computation_offset.to_le_bytes()],
        bump,
    )]
...
#[queue_computation_accounts("place_bid_v2", bidder)]
#[derive(Accounts)]
#[instruction(computation_offset: u64)]
pub struct PlaceBid<'info> {
    ...
    /// CHECK: computation_account
    pub computation_account: UncheckedAccount<'info>,
    #[account(address = derive_comp_def_pda!(COMP_DEF_OFFSET_PLACE_BID_V2))]
    pub comp_def_account: Box<Account<'info, ComputationDefinitionAccount>>,
    #[account(mut, address = derive_cluster_pda!(mxe_account, ErrorCode::ClusterNotSet))]
    pub cluster_account: Box<Account<'info, Cluster>>,
    #[account(mut, address = ARCIUM_FEE_POOL_ACCOUNT_ADDRESS)]
...
#[callback_accounts("place_bid_v2")]
#[derive(Accounts)]
pub struct PlaceBidV2Callback<'info> {
    pub arcium_program: Program<'info, Arcium>,
    #[account(address = derive_comp_def_pda!(COMP_DEF_OFFSET_PLACE_BID_V2))]
    pub comp_def_account: Account<'info, ComputationDefinitionAccount>,
    #[account(address = derive_mxe_pda!())]
...
#[init_computation_definition_accounts("place_bid_v2", payer)]
#[derive(Accounts)]
...
pub enum ErrorCode {
    ...
    #[msg("Unauthorized")]
    Unauthorized,
    #[msg("Auction MPC state is not initialized yet")]
    AuctionStateNotReady,
}
